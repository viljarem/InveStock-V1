...
def beregn_avanserte_features(df):
    ...
    try:
        ...
        # Kortsikt-returns (sterkeste enkelt-prediktor)
        for period in [1, 3, 5]:
            df[f'Return_{period}d'] = close.pct_change(period) * 100
            df[f'Return_{period}d'] = df[f'Return_{period}d'].clip(-30, 30)
        ...
        # --- FEATURE-INTERAKSJONER ---
        # RSI × Volum (oversold + volum-spike = sterk reversal-signal)
        if 'RSI' in df.columns:
            df['RSI_x_VolRatio'] = ((50 - df['RSI'].clip(0, 50)) / 50) * df.get('Vol_Ratio_20', pd.Series(1.0, index=df.index))
            df['RSI_x_VolRatio'] = df['RSI_x_VolRatio'].clip(0, 5)
        
        # Trend-styrke: Pris over SMA × ADX
        if 'Price_SMA_50_Ratio' in df.columns and 'ADX' in df.columns:
            df['Trend_Strength'] = (df['Price_SMA_50_Ratio'] - 1) * df['ADX'] / 50
            df['Trend_Strength'] = df['Trend_Strength'].clip(-3, 3)
        
        # Pengestrøm-posisjon: MFI × BB posisjon
        if 'MFI' in df.columns and 'BB_PctB' in df.columns:
            df['MFI_x_BBPos'] = (df['MFI'] / 100) * df['BB_PctB']
            df['MFI_x_BBPos'] = df['MFI_x_BBPos'].clip(0, 1)
...
def velg_features():
    ...
    base_features = [
        ...
        # Momentum
        'RSI', 'ROC_5', 'ROC_10', 'ROC_20', 'MOM_10', 'MOM_20',
        'Williams_R', 'RSI_Divergence',
        # Volatilitet
        'ATR_Norm', 'BB_Width', 'BB_PctB', 'HV_10', 'HV_20', 'Keltner_Pos',
        # Volum
        'Vol_Ratio_20', 'Vol_Ratio_50', 'Vol_Trend', 'OBV_Trend', 'MFI',
        # Trend
        'Price_SMA_50_Ratio', 'Price_SMA_200_Ratio', 'SMA50_SMA200_Ratio',
        'ADX', 'DI_Diff',
        # MACD
        'MACD_Hist_Change', 'MACD_Signal_Dist',
        # Pris mønstre
        ...
        'Ichimoku_Signal',
        # Feature-interaksjoner
        'RSI_x_VolRatio', 'Trend_Strength', 'MFI_x_BBPos',
    ]
...
class EnsembleStockPredictor:
    ...
    def __init__(self, horisont=10, target_pct=0.04, stop_pct=None, min_adx=10, min_hv=5.0, min_atr_norm=0.3):
    ...
    def fit(self, df, validate=True):
        ...
        try:
            ...
            if use_calibration:
                ...
                # Tren base-modeller på holdout for kalibrering
                calib_models = {
                    'xgb': xgb.XGBClassifier(
                        n_estimators=200, max_depth=4, learning_rate=0.05,
                        subsample=0.8, colsample_bytree=0.7,
                ...
                for name, mdl in calib_models.items():
                    mdl.fit(X_train_scaled, y_train, sample_weight=sw)
                
                # Kalibrer og finn terskel
                self.calibrators = {}
                temp_models = calib_models
                ...
                self.models = temp_models  # midlertidig for terskel-beregning
                try:
            ...
            # === FINAL: Tren på ALL data (gir sterkere modell) ===
            sw_all = self._compute_sample_weights(y)
            X_all_clean = X.replace([np.inf, -np.inf], np.nan).fillna(0)
            self.scaler.fit(X_all_clean)
            X_all_scaled = self.scaler.transform(X_all_clean)
            ...
            self.models['xgb'] = xgb.XGBClassifier(
                ...
                reg_alpha=0.1, reg_lambda=1.0,
                verbosity=0, use_label_encoder=False, random_state=42,
                n_jobs=1, scale_pos_weight=max(1.0, sw_all.max())
            )
            self.models['xgb'].fit(X_all_scaled, y, sample_weight=sw_all)
            
            self.models['rf'] = RandomForestClassifier(
                n_estimators=200, max_depth=6, min_samples_split=8,
                min_samples_leaf=4, random_state=42, n_jobs=1
            )
            self.models['rf'].fit(X_all_scaled, y, sample_weight=sw_all)
            
            self.models['gb'] = GradientBoostingClassifier(
                n_estimators=200, max_depth=4, learning_rate=0.05,
                subsample=0.8, random_state=42
            )
            self.models['gb'].fit(X_all_scaled, y, sample_weight=sw_all)
